...
from data_processor import data_cleaning, DataProcessor
import json
import keyboard
import pretty_midi
...
def upload_csv(file):
    try:
        ...
        with open(file_path, 'rb') as f:
            ...
            response = requests.post('http://####:5000/upload_csv', files=files)
...
def is_debug_mode():
    parser = argparse.ArgumentParser(description="妙妙工具箱")
    parser.add_argument('--debug', action='store_true', help='启用调试模式并显示控制台')
...
class GameAutomationAPI:
    def auto_upload(self):
        ...
        try:
            ...
            server_name = DataProcessor.extract_server_name(server_info)
            send_message(f"当前服务器: {server_name}")
